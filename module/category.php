...
switch ($action) { //$action variable defined in session.php
  case ACTION_FIND:
    category_read($request);
    break;
  case ACTION_READ:
    category_read($request);
    break;
  case ACTION_MOD:
    category_update($request);
    break;
  case ACTION_ADD:
    category_add($request);
    break;
  case ACTION_DEL:
    category_del($request);
    break;
  default:
...
function category_add() {
  ...
  require_once 'db.php';
  return db_insert(TBL_CATEGORY, $field_list, $args);
}
  ...
  if (!is_set_any($args, $field_list)) {
    ...
    foreach ($args as $arg) {
      if (is_array($arg) && is_set_any($arg, $field_list)) {
        db_delete(TBL_CATEGORY, $field_list, $arg, false);
        $succ_count++;
      } 
    }
    if ($succ_count) {
    ...
    return db_delete(TBL_CATEGORY, $field_list, $args);
  }
}
...
function category_read() {
  ...
  require_once 'db.php';
  return db_read(VIEW_CATEGORY, $args);
}
...
function category_update() {
  ...
  require_once 'db.php';
  return db_update(TBL_CATEGORY, array(FIELD_ID), $args);
}
?>